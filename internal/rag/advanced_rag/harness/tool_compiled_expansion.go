...
// NewCompiledExpander builds a CompiledExpander from a CompiledStore and the
// session's bound datasets. Mirrors Python, where the expander is constructed
// with the tools object that carries kb_ids / tenant. Nil store yields a nil
// expander (expansion disabled), matching SearchDeps.Expand == nil.
func NewCompiledExpander(store CompiledStore, kbIDs []string, tenantID string) CompiledExpander {
	if store == nil {
		return nil
	}
	return &compiledExpander{store: store, kbIDs: kbIDs, tenantID: tenantID}
}
...
// kgScopes mirrors Python _kg_scopes: turn a (possibly empty) doc_scope into a
// list of (kb_id, tenant_id, doc_ids) triples, one per bound dataset.
func (e *compiledExpander) kgScopes(docScope []string) []compiledScope {
	out := make([]compiledScope, 0, len(e.kbIDs))
	for _, kbID := range e.kbIDs {
		out = append(out, compiledScope{kbID: kbID, tenantID: e.tenantID, docIDs: docScope})
	}
	return out
}
...
func (e *compiledExpander) loadByDoc(ctx context.Context, sc compiledScope, order []string, byDoc map[string][]string, seen map[string]bool, maxChunks int) []map[string]any {
	...
	for _, docID := range order {
		...
		rows, err := e.store.LoadChunks(ctx, sc.kbID, sc.tenantID, cids)
		if err != nil {
			continue
		}
		for _, c := range rows {
