...
func NarrowByKeywords(chunks []map[string]any, keywords string) []map[string]any {
	...
	for _, c := range chunks {
		...
		// Narrowed text is stored under BOTH keys: chunkText prefers
		// content_with_weight, but some downstream readers only look at
		// "content" (and vice versa), and a stale full copy under the other key
...
// IsFactDenseSentence reports whether a sentence carries a fact-bearing signal:
// a number, a quoted span, a proper noun, or at least 6 tokens. Mirrors the
// Python grep_sed heuristic (number / _PROPER_NOUN_RE / length) used to keep
// only informative sentences when grepping the compiled structure.
func IsFactDenseSentence(s string) bool {
	...
	if properNounPattern.MatchString(s) {
		return true
	}
...
var (
	...
	// properNounPattern mirrors Python _PROPER_NOUN_RE: a capitalized word of at
	// least three letters. Python's _is_fact_dense_sentence keeps a sentence when
	// it carries a proper noun, so entity answers survive NarrowContent's
	// fact-density gate; the Go port must do the same or it drops named entities.
