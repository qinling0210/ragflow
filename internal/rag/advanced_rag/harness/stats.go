...
// The names match the Python-side @in_phase(...) labels exactly, so a Go run and
// a Python run emit comparable usage tables.
...
type LLMUsageStats struct {
	...
	roundStarts   map[string]float64
	currentRound  int
	phaseActive   map[string]int
	phaseStarts   map[string]float64
	roundPhaseAct map[phaseRoundKey]int
	roundPhaseSt  map[phaseRoundKey]float64
}
...
func (s *LLMUsageStats) RecordRound(phaseName string) {
	...
	if prev, ok := s.roundStarts[phaseName]; ok {
		s.roundTimes[phaseName] = append(s.roundTimes[phaseName], now-prev)
	}
...
func (s *LLMUsageStats) notePhaseExit(phaseName string, entryRound int) {
	...
	if active := s.phaseActive[phaseName]; active > 0 {
		...
		if active == 0 {
			...
			s.phaseTimeMs[phaseName] += now - start
