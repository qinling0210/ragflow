...
func TestParseISODate(t *testing.T) {
	...
	for _, bad := range []string{"1941-07", "not-a-date", "1941-07-28-01"} {
...
func TestExecutorCalculate(t *testing.T) {
	...
	if entry["value"] != "44" {
		t.Errorf("value = %v, want 44", entry["value"])
	}
	if entry["label"] != "years between" {
		t.Errorf("label = %v", entry["label"])
	}
	...
	if oc, _ := ex.Execute(context.Background(), "calculate", map[string]any{"question": "q"}); oc.Reason != ReasonBadArgs {
		t.Errorf("no facts: reason = %s, want bad_args", oc.Reason)
	}
}
