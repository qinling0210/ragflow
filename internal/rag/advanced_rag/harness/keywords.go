...
func ExtractWeightedKeywords(ctx context.Context, model SessionModel, question string) (string, string) {
	...
	if model != nil {
		msgs := []schema.Message{
			*schema.SystemMessage(KeywordsSystem),
	...
	// Cap term count to mirror Python _compact_keywords (max 15 terms). Go's
	// aspect-structured parsing already dedups across categories, so the cap is a
	// guard against an LLM emitting an oversized single aspect — it keeps the
	// query/keywords from ballooning the way Python's synonym runs did before the
	// same cap was added there.
	const maxKeywordTerms = 15
	if len(union) > maxKeywordTerms {
		union = union[:maxKeywordTerms]
		keywords = strings.Join(union, ", ")
	}
	if len(weighted) > maxKeywordTerms {
		weighted = weighted[:maxKeywordTerms]
		if len(weighted) == 0 {
			query = keywords
		} else {
			query = strings.Join(weighted, ", ")
