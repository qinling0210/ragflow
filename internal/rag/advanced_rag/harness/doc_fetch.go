...
const (
	...
	// estimateCharsPerToken approximates the tokenizer; it is only used to
	// enforce a rough budget, never for exact accounting.
	estimateCharsPerToken = 4
)

// fetchFullDocument mirrors Python RAGTools.fetch_full_document
// (agentic_rag.py:761): read a document end-to-end in reading order, in pages,
...
// (agentic_rag.py:933): load the whole document, fold it into the evidence the
