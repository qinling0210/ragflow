...
// MemorySearch mirrors Python memory.search: relevance-ranked retrieval over
// the raw-chunk memory store (used as a retrieval-reuse cache, NOT as a
// noise-injection source). Unlike MemoryGrep (loose keyword hit), it scores
// each memory chunk by how many of the query's SIGNIFICANT terms it actually
// contains and keeps only chunks that clear a normalized overlap bar — quality
// comparable to a knowledge-base retrieval but served from memory so a fact
// retrieved earlier can be reused instead of re-querying the index.
//
...
// significant terms; results are ranked by hit count then text length and
// capped at topN. Returns nil when nothing clears the bar (caller falls back
// to a knowledge-base search). minRatio <= 0 falls back to memoryMinRatio.
