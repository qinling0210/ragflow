...
// gets wrong often enough to matter. So the LLM writes ONE expression and we
// evaluate it.
//
// The expression comes from a language model, so it is NOT trusted. Go has no
// eval, and the LLM writes PYTHON syntax (`**`, `x if y else z`, list literals),
// which go/parser cannot read — so this file ships a small parser for a
// deliberately tiny Python subset, and rejects everything outside the whitelist
// BEFORE evaluation. There is no reflection, no name lookup, no property access:
// an unlisted construct is a parse error, not a runtime sandbox escape.
//
// Public interface
//
...
func extrema(op string, values []any) (any, error) {
	best, err := asNumber(values[0])
	if err != nil {
		return nil, err
	}
	bestInt, isInt := values[0].(int64)
	for _, v := range values[1:] {
		...
		if (op == "min" && f < best) || (op == "max" && f > best) {
			best = f
			bestInt, isInt = 0, false
			if i, ok := v.(int64); ok {
				bestInt, isInt = i, true
			}
		}
	}
	if isInt {
...
func sortValues(values []any) ([]any, error) {
	...
	for _, v := range out {
		if _, err := asNumber(v); err != nil {
			return nil, err
		}
	}
	for i := 1; i < len(out); i++ {
		for j := i; j > 0; {
			a, _ := asNumber(out[j-1])
			b, _ := asNumber(out[j])
			if a <= b {
				break
			}
