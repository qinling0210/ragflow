...
// chunkText must prefer "content" over "content_with_weight", mirroring Python
// memory._chunk_text, so a chunk carrying both yields the same text as Python.
func TestChunkTextPrefersContent(t *testing.T) {
	c := map[string]any{
		"content":             "primary text",
		"content_with_weight": "weighted text",
	}
	if got := chunkText(c); got != "primary text" {
		t.Errorf("chunkText = %q, want %q (Python prefers content)", got, "primary text")
	}

	// Falls back to content_with_weight when content is absent/empty.
	c2 := map[string]any{"content_with_weight": "weighted only"}
	if got := chunkText(c2); got != "weighted only" {
		t.Errorf("chunkText = %q, want %q", got, "weighted only")
	}
}
