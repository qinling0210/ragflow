...
func SufficientContextAgent(ctx context.Context, deps SCADeps, question string, claims []ClaimDraft) SCAResult {
	...
	prompt := renderPrompt(deps.Prompts, "sca_select", fallbackSCAReview, map[string]string{
		"question":       question,
...
// scaChunkText mirrors Python sufficient_context.py:188: prefer the reranked
...
func boundedExcerpt(text, hints string, maxChars int) string {
	...
	if start < 0 {
		...
		tail := maxChars / 2
		return text[:maxChars-tail] + " … " + text[len(text)-tail:]
	}
	half := maxChars / 2
	left := max(0, start-half)
	right := min(len(text), left+maxChars)
