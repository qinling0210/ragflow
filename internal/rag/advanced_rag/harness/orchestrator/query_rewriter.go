...
//
// It turns the SCA's forward gaps (what is missing + a search hint) into
// concrete, retrievable queries. Unlike simply reusing the gap text verbatim as
// the next claim description, this produces a targeted query that names the
// missing entity + relation, so the next search hits the gap instead of
// re-searching the same angle.
//
// The rewrite is deliberately cheap and only invoked on replan (whose budget is
...
func RewriteGapToQuery(ctx context.Context, deps RewriteDeps, question string, gaps []MissingPiece) []map[string]string {
	...
	prompt := renderPrompt(deps.Prompts, "sca_query_rewrite", fallbackQueryRewrite, map[string]string{
		"question":         question,
		"gaps":             gapsText,
